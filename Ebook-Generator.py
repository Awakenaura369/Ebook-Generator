...
class AIPowerhouse:
    ...
    def generate_introduction(self, book_info, style):
        ...
        return self.call_ai(system_prompt, user_prompt, self.models["llama3_70b"], 3000, 0.8)
    ...
    def generate_conclusion(self, book_info, style):
        ...
        return self.call_ai(system_prompt, user_prompt, self.models["mixtral"], 2500, 0.85)
...
class BookDatabasePro:
    ...
    def save_book(self, book_data, user_hash):
        ...
        self.conn.execute('''INSERT OR REPLACE INTO user_analytics 
            (user_hash, total_books, total_words, estimated_earnings, last_active)
            VALUES (?, 
                COALESCE((SELECT total_books FROM user_analytics WHERE user_hash = ?), 0) + 1,
                COALESCE((SELECT total_words FROM user_analytics WHERE user_hash = ?), 0) + ?,
                COALESCE((SELECT estimated_earnings FROM user_analytics WHERE user_hash = ?), 0) + ?,
                ?)''',
            (user_hash, user_hash, user_hash, word_count, user_hash, estimated_value, datetime.now())
        )
...
<body>
    <div class="book-container">
        ...
        <div class="content">
            {ExportManager.markdown_to_html(book_data.get('full_content', ''))}
        </div>
    ...
    @staticmethod
    def to_pdf_instructions():
        """Generate PDF creation instructions"""
        
        return """PDF CREATION INSTRUCTIONS:

1. **CHROME/EDGE METHOD (Recommended):**
...
2. **PROFESSIONAL TOOLS:**
   - **Canva:** Upload HTML, use their book templates
   - **Adobe InDesign:** Import HTML, professional typesetting
   - **Atticus/Reedsy:** Specialized book formatting tools

3. **AMAZON KDP REQUIREMENTS:**
   - Trim Size: 6x9 inches (most popular)
   - Bleed: 0.125 inches (if full-bleed)
   - Margins: 0.5 inches minimum
   - Font Size: 11-12pt for body text
   - Format: PDF/X-1a:2001 recommended

4. **PRINT-READY CHECKLIST:**
   ☐ Page numbers included
   ☐ Chapter starts on right-hand pages
   ☐ No widows/orphans
   ☐ Consistent spacing
   ☐ High-resolution images (300 DPI)
   ☐ CMYK color mode (for color printing)

5. **EBOOK OPTIMIZATION:**
   - Use reflowable EPUB for Kindle
   - Include table of contents with links
   - Add metadata (author, ISBN, keywords)
   - Test on multiple devices

For best results, consider professional formatting services ($100-$300)."""
